...
Keep responses concise, cite inline, and end with 'TERMINATE' when you have delivered the findings."""
    ...
    # NOTE: Groq OSS models do not reliably support OpenAI-style tool/function calling.
    # To avoid tool_call validation errors, we omit tools here and let the model
    # answer directly from its knowledge. Re-enable tools if using a model
    # that supports OpenAI tool calling.
    researcher = AssistantAgent(
        name="Researcher",
        model_client=model_client,
        tools=[],
        description="Gathers evidence from knowledge and reasoning (tools disabled for Groq).",
        system_message=system_message,
    )
...
Provide constructive but thorough feedback. End your evaluation with either \"TERMINATE\" if approved, or suggest specific improvements and then \"TERMINATE\"."""
